...
void RayTracer::rayTrace(ImageRGBf &img, int numReflection) const
{
...
Vec RayTracer::trace(const Ray ray, int num_reflection) const
{
  Ray ray_reflected;
  bool reflection;
...
bool RayTracer::closestPoint(/*in*/ const Ray ray,
...
Vec RayTracer::shade(/*in*/ const Ray ray,
                     ...
                     /*out*/ bool &reflection) const
{
  ...
  for (int ch = 0; ch < 3; ch++)
  {
    color[ch] = world.lightEnv * world.ka * obj->material.color[ch] +
                fatt * source.color[ch] * (obj->material.kd * obj->material.color[ch] * cosTetha + obj->material.ks * pow(cosPhi, obj->material.n_shiny));
  }
